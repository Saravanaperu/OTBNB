...
class MomentumBreakoutStrategy(BaseStrategy):
    ...
    def generate_signal(
        ...
        is_bullish_breakout = (
            snapshot.close > snapshot.morning_high and body_percentage > 0.003
        )
        is_bearish_breakout = (
            snapshot.close < snapshot.morning_low and body_percentage > 0.003
        )
    ...
    def should_exit(
        ...
        if position.side == "BUY_CE":
            if (
                ...
                or snapshot.supertrend_direction == "DOWN"
            ):
                return ExitSignal(
