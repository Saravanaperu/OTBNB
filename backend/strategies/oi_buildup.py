    ...
    def should_exit(self, position: Position, snapshot: MarketSnapshot) -> Optional[ExitSignal]:
        ...
        if position.side == 'BUY_CE':
            if snapshot.close < snapshot.vwap:
                return ExitSignal(symbol=position.symbol, token=position.token, reason="Price below VWAP")
        elif position.side == 'BUY_PE':
            if snapshot.close > snapshot.vwap:
                return ExitSignal(symbol=position.symbol, token=position.token, reason="Price above VWAP")
