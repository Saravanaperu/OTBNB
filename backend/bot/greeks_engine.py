...
from py_vollib.black_scholes.implied_volatility import PriceIsBelowIntrinsic, PriceIsAboveMaximum
...
class GreeksEngine:
    ...
    def refresh(self, snapshot: Dict[str, Any], spot_price: float, time_to_expiry: float, risk_free_rate: float = 0.1):
        ...
        for token, option_data in snapshot.items():
            ...
            try:
                ...
                # Calculate Greeks
                delta = greeks.delta(flag, spot_price, strike, time_to_expiry, risk_free_rate, implied_vol)
                gamma = greeks.gamma(flag, spot_price, strike, time_to_expiry, risk_free_rate, implied_vol)
                theta = greeks.theta(flag, spot_price, strike, time_to_expiry, risk_free_rate, implied_vol)
                vega = greeks.vega(flag, spot_price, strike, time_to_expiry, risk_free_rate, implied_vol)
                ...
                logger.debug(f"Volatility calc error for strike {strike} {flag}: {str(e)}")
