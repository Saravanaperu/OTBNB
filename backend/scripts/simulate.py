...
async def run_simulation():
    ...
    risk_manager = RiskManager({"daily_loss_limit": 5000, "max_open_positions": 5, "risk_per_trade": 1000})
    ...
    logger.info("Portfolio state", open_trades=portfolio_manager.get_open_trades_count(), mtm=portfolio_manager.get_total_mtm())

    # Simulate price update
    logger.info("Updating prices")
    portfolio_manager.update_prices({"100": 60.0})
    logger.info("Portfolio state after update", open_trades=portfolio_manager.get_open_trades_count(), mtm=portfolio_manager.get_total_mtm())

    # Simulate exit
    logger.info("Exiting position")
    portfolio_manager.remove_position("100", 65.0)
    logger.info("Final portfolio state", open_trades=portfolio_manager.get_open_trades_count(), mtm=portfolio_manager.get_total_mtm())
